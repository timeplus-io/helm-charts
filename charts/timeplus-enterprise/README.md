...
![Version: 1.0.0](https://img.shields.io/badge/Version-1.0.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 3.2.4-rc.1](https://img.shields.io/badge/AppVersion-3.2.4--rc.1-informational?style=flat-square)
...
| timeplusAppserver.encryptionKey | string | `""` | Passphrase that encrypts Data Agent credentials (LLM API key, MCP server secrets) at rest. The chart stores it in a Secret and exports it to the appserver as `NEUTRON_ENCRYPTION_KEY`. Leave empty to use `existingEncryptionKeySecret` instead. With neither set, credentials are sealed with a built-in default key that only obfuscates them. Back the key up: losing it orphans every sealed credential, which must then be re-entered. |
| timeplusAppserver.existingEncryptionKeySecret | string | `""` | Name of an existing Secret in the release namespace whose `NEUTRON_ENCRYPTION_KEY` key holds the passphrase. Preferred for production and GitOps, since the passphrase never enters a values file. Mutually exclusive with `encryptionKey`. |
| timeplusAppserver.extraContainers | list | `[]` | Extra containers that to be run together with the main container. |
| timeplusAppserver.extraEnv | list | `[]` | Extra environment variables for the appserver container, as a list of EnvVar objects (e.g. `- name: FOO` / `value: bar`, or `valueFrom`). |
